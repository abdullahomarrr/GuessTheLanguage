...
function cors(origin: string | null) {
  const allowed = (process.env.LINGO_ALLOWED_ORIGINS || 'http://localhost:3000').split(',').map((item) => item.trim());
...
export async function POST(request: Request) {
  ...
  if (!secret || secret.length < 32) return Response.json({ error: 'Ingestion is not configured.' }, { status: 503, headers });
  ...
  return Response.json({ accepted: true, persisted: Boolean(sql) }, { status: 202, headers });
}
